...
class MovieList extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      `https://api.themoviedb.org/3/movie/${this.state.movieType}?api_key=33cff2b0fc5d808790ffb3ba68053642&language=en-US&page=1`
    )
      .then((res) => res.json())
      .then(
        (result) => {
  ...
  render() {
    const movieList = this.state.movies.map(
      ...
      return (
        <div>
          <h1 className='section-title'>
            {this.capitalizeFirstLetter(movieType)}
          </h1>
